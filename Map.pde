class Map
{
  ...
  { 
    ...
    line(width_margin+box_size-3, height-height_margin*2-2, width_margin+box_size-3, height-height_margin*2 +5);
    // Right side verticle purple line
    line(width_margin+((int)no_boxes*box_size)+3, height-height_margin*2-2, width_margin+((int)no_boxes*box_size)+3, height-height_margin*2 +5);
    
    // Floor line
    strokeWeight(5);
    line(width_margin+box_size-2, height-height_margin*2+9, width_margin+((int)no_boxes*box_size)+2, height-height_margin*2+9);
    
    // bottom left angled lines
    strokeWeight(3);
    for(int k=1; k<no_boxes; k++)
    {
      line(width_margin+(k*box_size), height-height_margin*2+9, width_margin+(k*box_size)-10, height-height_margin*2 +25);
    } // end little for
  }
}